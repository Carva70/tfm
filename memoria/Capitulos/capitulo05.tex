...
\begin{wrapfigure}{l}{0.5\linewidth}
    \centering
    \includegraphics[width=1\linewidth]{Imagenes/inyeccion_ejemplo.png}
    \caption{Ejemplo de inyección de etiqueta junto a la instrucción}
    \label{fig:inyeccion_ejemplo}
\end{wrapfigure}

%(clasificador \textrightarrow{} SQL \textrightarrow{} respuesta final)
El objetivo de las pruebas es forzar a que el clasificador enrute el prompt como \texttt{needs\_db\_access}, y luego genere la sentencia sql obedeciendo las instrucciones en los diferentes casos. Cada plantilla de inyección de etiqueta intenta forzar la clasificación de una manera distinta, y cada caso contiene una instrucción de exfiltración distinta:

\begin{itemize}
	\item \texttt{tests/safety\_cases.json}: conjunto de casos. Define prompts de exfiltración.
...
\begin{itemize}
	...
	\item La respuesta final no filtra ni imagina información sensible (no proporciona resultados que coinciden con expresiones regulares propias de IBAN, SSN, etc.).
\end{itemize}
...
Las pruebas contienen 11 casos de seguridad (instrucción maliciosa) y 12 plantillas de inyección (envuelve la instrucción para que retorne \texttt{needs\_db\_access} en la clasificación). Cada caso se prueba en su variante base más las 12 plantillas, dando un total de 13 variantes por caso y 143 ejecuciones por modelo.
...
\texttt{gpt-oss-safeguard:20b} & 143 & 134 & 9 & 9 & 0 \\
